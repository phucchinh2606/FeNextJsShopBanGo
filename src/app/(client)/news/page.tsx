...
export default function NewsListPage() {
  ...
  return (
    <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
      {/* Banner tiêu đề */}
      <div className="bg-gradient-to-r from-amber-900 to-amber-950 text-white p-6 sm:p-12 rounded-2xl sm:rounded-3xl shadow-sm mb-6 sm:mb-10 relative overflow-hidden">
        <div className="relative z-10 max-w-2xl">
          <span className="text-amber-400 text-[10px] sm:text-xs font-semibold uppercase tracking-wider">
            Kiến thức & Kinh nghiệm
          </span>
          <h1 className="text-2xl sm:text-4xl font-serif font-bold mt-1.5 sm:mt-2 leading-tight">
            Tin Tức & Kiến Thức Nội Thất Gỗ
          </h1>
          <p className="text-amber-100/80 text-xs sm:text-sm mt-2 sm:mt-3 leading-relaxed">
      ...
      {/* Thanh tìm kiếm & Lọc */}
      <div className="bg-white p-3.5 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 mb-6 sm:mb-8">
        ...
        <p className="text-xs text-slate-500 font-medium self-end sm:self-center">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="h-80 sm:h-96 bg-slate-200/60 rounded-2xl animate-pulse"
            />
          ))}
        </div>
      ) : newsList.length === 0 ? (
            ...
            return (
              ...
              >
                ...
                <div className="p-5 sm:p-6 flex flex-col flex-1 justify-between">
                  <div>
                    <div className="flex items-center space-x-4 text-[11px] text-slate-400 mb-2.5 sm:mb-3">
                    ...
                    <Link href={`/news/${item.newsId}`}>
                      <h2 className="text-sm sm:text-base font-bold text-slate-900 group-hover:text-amber-900 transition line-clamp-2 leading-snug">
                        {item.title}
                      </h2>
                    </Link>

                    <p className="text-xs text-slate-500 mt-2 sm:mt-2.5 line-clamp-3 leading-relaxed">
                      {item.summary}
                    </p>
                  </div>
      ...
      {/* Phân trang */}
      {totalPages > 1 && (
        <div className="flex items-center justify-center space-x-2 pt-8 sm:pt-10">
          <button
            disabled={pageNumber === 1}
            onClick={() => setPageNumber((prev) => Math.max(prev - 1, 1))}
            className="p-2 bg-white border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50 disabled:opacity-40 transition"
          >
