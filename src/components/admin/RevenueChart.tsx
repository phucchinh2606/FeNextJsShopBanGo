...
// Custom Tooltip hiển thị đẹp mắt khi hover
...
export const RevenueChart = ({
  ...
  if (isLoading) {
    return <div className="h-72 bg-slate-100 rounded-xl animate-pulse" />;
  }

  if (!dataItems || dataItems.length === 0) {
    return (
      <div className="h-72 flex items-center justify-center text-xs text-slate-400 border border-dashed rounded-xl">
        Chưa có dữ liệu thống kê doanh thu
      </div>
    );
  }

  // Rut ngan con so cho YAxis (vi du: 1.5M, 20M)
